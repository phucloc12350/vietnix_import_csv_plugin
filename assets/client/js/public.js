...
(function ($) {
  ...
  window.VietnixConfig = {
    ...
    getPluginUrl: function (tableId) {
      ...
      if (!tableId) {
        ...
        if (firstTableId) {
          return (
            vietnixTables[firstTableId].VIETNIX_IMPORT_CSV_PLUGIN_URL || ""
          );
        }
        return "";
      }
      ...
      return config ? config.VIETNIX_IMPORT_CSV_PLUGIN_URL : "";
    },
  ...
  function initTabs($wrapper, config) {
    ...
    $wrapper.on("click", ".vietnix-server-tab", function () {
      ...
      $wrapper
        .find(`.vietnix-usage-tabs[data-server-content="${server}"]`)
        .addClass("active");

      // Reset to first usage time for this server
      const $firstUsageTab = $wrapper.find(
        `.vietnix-usage-tabs[data-server-content="${server}"] .vietnix-usage-tab:first`
      );
      ...
      vietnixTables[config.tableId].filters.usage =
        $firstUsageTab.data("usage") || "";
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response.success) {
          ...
          showError(
            $wrapper,
            response.data.message || "Có lỗi xảy ra khi tải dữ liệu"
          );
        }
      },
  ...
  function buildProductCard(item, config) {
    ...
    if (
      item.price_sale_raw &&
      parseFloat(item.price_sale_raw) > 0 &&
      parseFloat(item.price_sale_raw) < parseFloat(item.price_raw)
    ) {
      html += '<span class="original-price">' + item.price + "</span>";
      html +=
        '<span class="sale-price"><span class="current-price-value">' +
        item.price_sale +
        "</span><span>/th</span></span>";
    } else {
      html +=
        '<span class="current-price"><span class="current-price-value">' +
        item.price +
        "</span><span>/th</span></span>";
    }
    html +=
      '<div class="img-sale"><img src="' +
      VietnixConfig.getAssetUrl("assets/image/icon_sale.png", config.tableId) +
      '"/></div>';
    html += "</div>";
    // Addon pricing
    if (item.price_addon_sale) {
      ...
      html +=
        '<span class="addon-price"><span>Giảm thêm</span>' +
        escapeHtml(item.price_addon_sale) +
        "</span>";
      if (item.price_addon_code) {
        html +=
          '<code class="addon-code">' +
          escapeHtml(item.price_addon_code) +
          "</code>";
        html +=
          '<span class="addon-copy" data-copy="' +
          escapeHtml(item.price_addon_code) +
          '" title="Sao chép"><img src="' +
          VietnixConfig.getAssetUrl(
            "assets/image/icon_copy.png",
            config.tableId
          ) +
          '"/></span>';
      }
    ...
    if (item.product_content) {
      ...
      const features = item.product_content
        .split(",")
        .map((f) => f.trim())
        .filter((f) => f);
      ...
      features.forEach((feature) => {
        ...
        if (parts.length === 2) {
          ...
          if (/^https?:\/\/.+\.(png|jpg|jpeg|gif|svg)$/i.test(parts[0])) {
            ...
            textHtml = `<span>${escapeHtml(parts[0])} ${escapeHtml(
              parts[1]
            )}</span>`;
          }
        } else {
    ...
    if (item.server_sale) {
      html +=
        '<div class="server-sale">' + escapeHtml(item.server_sale) + "</div>";
    }
    ...
    if (item.product_cta_link) {
      ...
      html +=
        '<a href="' +
        escapeHtml(item.product_cta_link) +
        '" class="cta-button" target="_blank">Xem chi tiết</a>';
      html += "</div>";
    }
  ...
  function showError($wrapper, message) {
    ...
    $productsGrid.html(
      `<div class="error-message" style="text-align: center; color: #d63638; padding: 40px;">Error: ${escapeHtml(
        message
      )}</div>`
    );
  }
