...
(function($) {
    ...
    $(document).ready(function() {
        ...
        initDataTable();
        initSettings();
        initImportPageForm(); // Thêm init cho import page form
    });
    ...
    // Import Page Form Handler (from import-page.php)
    function initImportPageForm() {
        ...
        $('#vietnix-csv-import-form').on('submit', function(e) {
            ...
            if (!$('#csv_file')[0].files.length) {
                alert(vietnixCSVAdmin.messages?.selectFile || 'Please select a CSV file');
                return;
            }
            
            // Confirm if clearing existing data
            if ($('#clear_existing').is(':checked')) {
                if (!confirm(vietnixCSVAdmin.messages?.confirmDelete || 'Are you sure you want to delete all existing data before import?')) {
                    return;
                }
            }
            ...
            $.ajax({
                url: ajaxurl || vietnixCSVAdmin.ajaxUrl,
                type: 'POST',
                data: formData,
                processData: false,
                contentType: false,
                ...
                error: function() {
                    alert(vietnixCSVAdmin.messages?.connectionError || 'Connection error. Please try again.');
                },
                complete: function() {
        ...
        $('#delete-all-data').on('click', function() {
            if (!confirm(vietnixCSVAdmin.messages?.confirmDeleteAll || 'Are you sure you want to delete ALL data? This action cannot be undone!')) {
                return;
            }
            
            $.post(ajaxurl || vietnixCSVAdmin.ajaxUrl, {
                action: 'vietnix_delete_data',
                nonce: vietnixCSVAdmin.nonce
            }, function(response) {
                if (response.success) {
                    alert(response.data || 'Data deleted successfully!');
                    location.reload();
                } else {
